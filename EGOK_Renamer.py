...
# Версия программы
VERSION = "3.9.9"  # Увеличили версию
...
class DatabaseManager:
        ...
        try:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS rename_history (
                    ...
                    is_active INTEGER DEFAULT 1  -- Флаг активности файла (1 - активен, 0 - удален)
                )
            ''')
            ...
            # Создаем индекс для поиска активных файлов
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_active 
                ON rename_history(is_active)
            ''')
            
            # Создаем индекс для поиска по новому имени
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_new_name 
                ON rename_history(new_name)
            ''')
    ...
    def add_record(self, timestamp, route, original_name, new_name, file_path):
        ...
        try:
            ...
            cursor.execute('''
                ...
                (timestamp, create_date, route, original_name, new_name, file_path, is_active)
                VALUES (?, ?, ?, ?, ?, ?, 1)
    ...
    def get_records_by_date(self, target_date=None, only_active=True):
        ...
        try:
            ...
            if target_date:
                if only_active:
                    cursor.execute('''
                        ...
                        WHERE create_date = ? AND is_active = 1
            ...
            columns = ['id', 'timestamp', 'create_date', 'route', 'original_name', 'new_name', 'file_path', 'is_active']
            result = []
    ...
    def get_record_by_new_name(self, new_name, only_active=True):
        """Получение записи по новому имени файла"""
        try:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            ...
            if only_active:
                cursor.execute('''
                    SELECT * FROM rename_history 
                    WHERE new_name = ? AND is_active = 1
                    ORDER BY timestamp DESC
                    LIMIT 1
                ''', (new_name,))
            else:
            ...
            logging.error(f"Ошибка получения записи по имени файла: {e}")
            return None
    
    def deactivate_record(self, new_name):
        """Деактивация записи по новому имени файла"""
        try:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            ...
            logging.error(f"Ошибка деактивации записи: {e}")
            return False
    
    def mark_record_inactive(self, file_path):
        """Пометить запись как неактивную (файл удален)"""
        try:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            ...
            logging.error(f"Ошибка обновления статуса записи: {e}")
            return False
    
    def get_max_counter_for_date(self, target_date, project, route, cn_type, date_format):
        """Получить максимальный счетчик для заданной даты и параметров"""
        try:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            
            # Получаем все записи за указанную дату
            cursor.execute('''
                SELECT new_name FROM rename_history 
                WHERE create_date = ? 
                ORDER BY timestamp DESC
            ''', (target_date,))
            ...
            # Создаем шаблон для поиска счетчика
            # Формат: {project}_{date}_{route}_{counter}_{CN}
            date_pattern = self._convert_date_format_for_regex(date_format)
            pattern = re.compile(
                f"{re.escape(project)}_{date_pattern}_"
                f"{re.escape(route)}_(\\d+)_{re.escape(cn_type)}"
            )
            ...
            logging.error(f"Ошибка получения максимального счетчика: {e}")
            return 0
    
    def _convert_date_format_for_regex(self, date_format):
        """Конвертирует формат даты в регулярное выражение"""
        # Заменяем форматы дат на соответствующие паттерны
        format_mapping = {
            "ДДММГГГГ": r"\d{8}",
            "ДДММГГ": r"\d{6}",
            "ГГГГММДД": r"\d{8}",
            "ДД.ММ.ГГГГ": r"\d{2}\\.\\d{2}\\.\\d{4}",
            "ДД.ММ.ГГ": r"\d{2}\\.\\d{2}\\.\\d{2}",
            "ГГГГ.ММ.ДД": r"\d{4}\\.\\d{2}\\.\\d{2}"
        }
        return format_mapping.get(date_format, r"\d{8}")
    
    def cleanup_missing_files(self):
        """Очистка записей о файлах, которые больше не существуют"""
        try:
            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            
            # Получаем все записи с is_active=1
            cursor.execute('''
                SELECT id, file_path FROM rename_history 
                WHERE is_active = 1
            ''')
            ...
            for record_id, file_path in records:
                if not os.path.exists(file_path):
                    # Помечаем запись как неактивную
                    cursor.execute('''
                        UPDATE rename_history 
                        SET is_active = 0 
                        WHERE id = ?
            ...
            if cleaned_count > 0:
                logging.info(f"Очищено {cleaned_count} записей о несуществующих файлах")
...
class TemplateBuilderDialog:
    ...
    def update_preview(self):
        ...
        try:
            ...
            # Форматируем дату по выбранному форматату
            date_format = self.settings.settings.get("date_format", "ГГГГММДД")
            ...
            # УДАЛЕН {extension} - добавляем расширение в конец
            preview = preview + ".jpg"
...
class Settings:
    ...
    def add_to_template_history(self, template):
        ...
        if template and template not in self.settings["template_history"]:
            ...
            # Ограничиваем историю 10 элементами
            self.settings["template_history"] = self.settings.settings["template_history"][:10]
            self.save_settings()
...
class RenamerApp:
    ...
    def __init__(self, root):
        ...
        # Очистка базы данных от записей о несуществующих файлах при запуске
        self.db_manager.cleanup_missing_files()
    ...
    def create_settings_content(self, parent):
        ...
        # Привязываем события для обновления истории и предпросмотра
        ...
        self.template_cb.bind('<KeyRelease>', lambda e: self.update_preview())
        ...
        # ДОБАВЛЕНО: Поле предпросмотра имени файла
        preview_frame = ttk.Frame(rename_frame)
        ...
        # Обновляем предпросмотр при запуске
        self.update_preview()
    ...
    def update_preview(self, event=None):
        ...
        try:
            ...
            preview = preview.replace("{counter}", "001")
            # УДАЛЕН {extension} - расширение добавляется автоматически
            preview = preview + ".jpg"  # Просто добавляем расширение в конец
    ...
    def add_to_report(self, original_name, new_name, filepath, create_time=None):
        ...
        # Проверяем, есть ли уже запись с таким именем файла в отчете
        existing_record_index = -1
        for i, row in enumerate(self.report_data):
            if len(row) > 3 and row[3] == new_name:  # Проверяем колонку "new_name"
                existing_record_index = i
                break
        
        if existing_record_index >= 0:
            # Удаляем существующую запись из отчета
            del self.report_data[existing_record_index]
        ...
        if TKSHEET_AVAILABLE and hasattr(self, 'report_sheet'):
            ...
            values = (number, create_time, route, new_name)
        ...
        # Обновляем историю переименований
        self.rename_history = []
        for row in self.report_data:
    ...
    def generate_filename(self, filepath, counter=None):
        ...
        # Заменяем переменные в шаблоне (УДАЛЕН {extension})
        ...
        # УДАЛЕНА замена {extension}
        
        return f"{filename}.{file_ext}"  # Расширение добавляется в конце

    def get_next_counter(self):
        """Получение следующего номера счетчика - ИСПРАВЛЕННАЯ ВЕРСИЯ с учетом ТОЛЬКО существующих файлов"""
        folder = self.settings.settings["folder"]
        today = datetime.now()
        date_format = self.settings.settings.get("date_format", "ГГГГММДД")
        formatted_date = self.format_date_by_format(today, date_format)
        ...
        project = self.settings.settings["project"]
        route = self.settings.settings["route"]
        cn_type = self.settings.settings["cn_type"]
        
        # Создаем регулярное выражение для поиска файлов с текущими параметрами
        escaped_date = re.escape(formatted_date)
        pattern = re.compile(
            f"{re.escape(project)}_{escaped_date}_"
            f"{re.escape(route)}_(\\d+)_{re.escape(cn_type)}"
        )
        
        # Ищем ВСЕ существующие файлы в папке
        ...
        # Ищем минимальный свободный номер (дырку)
        max_num = max(existing_numbers)
        
        # Проверяем все номера от 1 до max_num
            ...
            with self.rename_lock:
                for filepath in filepaths:
                    try:
                        ...
                        # Деактивируем старую запись с таким же именем файла в базе данных
                        self.db_manager.deactivate_record(new_name)
...
• Сохранение порядкового номера даже после удаления файлов
• Автоматическое обновление отчета при повторном использовании имен
...
{{counter}} - Счетчик файлов (сохраняется даже после удаления файлов)
...
✓ Сохранение порядковых номеров файлов даже после их удаления
✓ Автоматическое обновление отчета при повторном использовании имен файлов
...
"""
    ...
    def on_closing(self):
        ...
        try:
            ...
            # Сохраняем настройки БЕЗ показа сообщения
